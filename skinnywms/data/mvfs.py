...
class MvLayer(datatypes.Layer):
    ...
    def load_style(self, conf):
        ...
        for style_conf in conf.get("styles"):
            ...
            if not style_verb:
                raise Exception(
                    "No _verb is specified for [%s] in style: %s" % (self, style_conf)
                )
            style_conf.pop("_verb")
            style_conf_lst.append(datatypes.StyleConfig(style_verb, style_conf))
...
class MvDataLayer(MvLayer):
    ...
    def dimensions(self):
        if self.fixed_layer:
            ...
            return [
                datatypes.Dimension(
                    "dim_index", "no", 0, "0/{}/1".format(len(self._fields) - 1)
                )
            ]
...
class MvAvailability(datatypes.Availability):
            ...
            with open(self._path, "rt") as f:
                ...
                for conf_def in conf.get("layers"):
                    ...
                    if layer:
                        ...
                        raise Exception("No layer defined in conf={}".format(conf_def))

                # self.log.info('layers {}'.format(self._layers))
        except IOError as e:
            raise ("Could not open config file: {}. Error: {}".format(self._path, e))
    ...
    def add_layer(self, layer_id, conf):
        ...
        if not layer_type:
            raise Exception("No type is specified for layer in conf={}".format(conf))
        ...
        if layer_type == "map":
            self._layers[layer_name] = MvMapLayer(
                self, self.context, layer_name, layer_name, zindex, conf
            )
        else:
            self._layers[layer_name] = MvDataLayer(
                self, self.context, layer_name, layer_name, zindex, conf
            )
